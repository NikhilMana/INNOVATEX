...
interface InfiniteGalleryProps {
	...
	scrollProgress?: any; // MotionValue<number> from framer-motion
}
...
function GalleryScene({
	...
	useFrame((state, delta) => {
		if (scrollProgress) {
			...
			const diff = currentProgress - lastScrollProgress.current;
			lastScrollProgress.current = currentProgress;
			
			// Transform small fractional progress differences into a noticeable velocity
			const targetVelocity = diff * 1500 * speed;
			
			// Smoothly interpolate towards the target velocity
			setScrollVelocity((prev) => THREE.MathUtils.lerp(prev, targetVelocity, 0.15));
		} else {
			// Apply auto-play
			if (autoPlay) {
